...
def get_weka_path(script_path):
    ...
    weka_path = os.environ.get("WEKA38")
    if weka_path is None:
        weka_path = os.path.join(script_path, 'weka-3-8-1', 'weka.jar')
    else:
    ...
    if not path_exists:
        ...
        print("Check the installation and the given path to the WEKA software {} in ApoplastP.py (line 36).".format(weka_path))
        print("Alternatively set the environment variable 'WEKA38' to point to the folder containing the weka.jar file.")
        print()
        sys.exit(1)
...
def get_emboss_path(script_path):
    ...
    if not path_exists:
        ...
        print("Check the installation and the given path to the EMBOSS software {} in ApoplastP.py (line 57).".format(emboss_path))
